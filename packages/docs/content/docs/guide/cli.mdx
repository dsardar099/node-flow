...
<Callout>
  **`nf` is not on npm in 1.0.** It reaches deeper into the engine than the
  published packages do — `migrate` and `bootstrap` talk to Postgres directly —
  so its surface is not yet one worth committing to as a public API. Run it from
  a clone, or from the container:

  ```bash
  docker run --rm -e NF_URL -e NF_API_KEY \
    ghcr.io/dsardar099/node-flow/server:1.0.0 nf workflows list
  ```
</Callout>
